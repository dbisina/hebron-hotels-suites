...
export default function EventsPage() {
  ...
  const inputCls = "w-full bg-[rgba(255,255,255,0.04)] border border-[rgba(255,255,255,0.09)] text-cream-100 text-sm px-4 py-3 outline-none focus:border-[#C9A84C]/50 transition-colors placeholder:text-cream-200/20";
    ...
    <div style={{ background: "#0D0704", minHeight: "100vh" }}>
      ...
      <section className="relative pt-40 pb-24 px-6 md:px-16">
        <div className="max-w-screen-lg mx-auto">
          ...
          <motion.p
            className="font-sans text-cream-200/45 text-sm leading-relaxed max-w-lg mt-8"
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 mb-20">
            {EVENT_TYPES.slice(0, -1).map((evt, i) => (
              <motion.button
                ...
                style={{
                  background: form.eventType === evt.key ? "rgba(201,168,76,0.07)" : "rgba(255,255,255,0.02)",
                  border: form.eventType === evt.key ? "1px solid rgba(201,168,76,0.45)" : "1px solid rgba(255,255,255,0.07)",
                }}
              >
                <p
                  className="font-display text-base mb-2"
                  style={{ color: form.eventType === evt.key ? "#C9A84C" : "#E8DFD0", fontWeight: 300 }}
                >
                  {evt.label}
                </p>
                <p className="text-xs text-cream-200/40 font-sans leading-relaxed">{evt.description}</p>
              </motion.button>
            ))}
          </div>
          ...
          >
            ...
            {status === "sent" ? (
              <div className="py-20 text-center">
                ...
                <p className="text-cream-200/45 text-sm max-w-sm mx-auto">
              ...
              <form onSubmit={submit} className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div suppressHydrationWarning>
                  <label className="block text-[9px] tracking-[0.3em] uppercase text-cream-200/40 mb-2">Full Name *</label>
                ...
                <div suppressHydrationWarning>
                  <label className="block text-[9px] tracking-[0.3em] uppercase text-cream-200/40 mb-2">Email Address *</label>
                ...
                <div suppressHydrationWarning>
                  <label className="block text-[9px] tracking-[0.3em] uppercase text-cream-200/40 mb-2">Phone Number</label>
                ...
                <div suppressHydrationWarning>
                  <label className="block text-[9px] tracking-[0.3em] uppercase text-cream-200/40 mb-2">Event Type *</label>
                  <select
                  ...
                  >
                    <option value="" disabled style={{ background: "#1A0E0A" }}>Select event type</option>
                    {EVENT_TYPES.map((t) => (
                      <option key={t.key} value={t.key} style={{ background: "#1A0E0A" }}>{t.label}</option>
                    ))}
                  </select>
                </div>
                <div suppressHydrationWarning>
                  <label className="block text-[9px] tracking-[0.3em] uppercase text-cream-200/40 mb-2">Preferred Date</label>
                  <input
                    suppressHydrationWarning
                    type="date"
                ...
                <div suppressHydrationWarning>
                  <label className="block text-[9px] tracking-[0.3em] uppercase text-cream-200/40 mb-2">Expected Guests</label>
                  <input suppressHydrationWarning value={form.guestCount} onChange={(e) => set("guestCount", e.target.value)} placeholder="e.g. 50-100" className={inputCls} />
                </div>
                <div suppressHydrationWarning className="md:col-span-2">
                  <label className="block text-[9px] tracking-[0.3em] uppercase text-cream-200/40 mb-2">Additional Details</label>
                  <textarea
                    suppressHydrationWarning
                    rows={5}
                    value={form.message}
                ...
                {status === "error" && (
                  <div className="md:col-span-2">
                    <p className="text-red-400/70 text-xs">Something went wrong. Please try again or call us directly.</p>
                  </div>
                )}
                ...
                <div className="md:col-span-2 flex items-center gap-6 mt-2">
                  <button
                    ...
                    style={{ background: "#C9A84C", color: "#0D0704", opacity: status === "sending" ? 0.7 : 1 }}
                  >
                    {status === "sending" ? "Submitting…" : "Submit Enquiry"}
                  </button>
                  <p className="text-xs text-cream-200/30 font-sans">
